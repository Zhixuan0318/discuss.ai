...
// Destination Caller
const destinationCaller = "0x6b0de2e666ba292fa8111dc161938535cb2e16b3";
const encodedDestinationCaller = `0x${destinationCaller.replace(/^0x/, '').padStart(64, '0')}`;
...
// Burn USDC with Hook: TokenMessengerV2 contract
export async function burn(sourceChain, destChain, amount, destAddress, walletID) {
    ...
    // Define the data
    const recipient = destAddress;
    const action = "MINT_BADGE";

    // Encode data into bytes
    const encodedHookData = abiCoder.encode(
        ["address", "string"],
        [recipient, action]
    );
...
export async function mint(messageBytes, attestation, walletID) {
    ...
    const requestBody = {
        ...
        idempotencyKey: idempotencyKey,
        contractAddress: destinationCaller,
        feeLevel: "LOW",
        walletId: walletID,
        entitySecretCiphertext: entitySecretCiphertext
    };
...
export default async function v2cctpWithHook(sourceChain, sourceWalletID, destChain, destWalletID, destWalletAddress, amount) {
    try {
        console.log("Starting CCTP V2 Transfer with Hooks...");
        ...
        // Step 2: Burn
        console.log("Burning USDC with hook data attached...");
        ...
        // Step 4: Mint
        console.log("Destination caller handling USDC minting on destination chain...");
        const messageBytes = attestation.message;
        const finalAttestation = attestation.attestation;
        const txReponse = await mint(messageBytes, finalAttestation, destWalletID);
        const txID = txReponse.data.id;
        await waitForTransactionCompletion(txID);
        console.log("Minting completed. CCTP V2 with Hook transfer successful! An NFT badge had minted on the destination wallet as a celebration!");

        return txID;

    } catch (error) {
        console.error("CCTP with Hook transfer failed:", error);
        throw error;
    }
}
